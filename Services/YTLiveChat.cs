...
internal class YTLiveChat : IYTLiveChat
{
    ...
    private static readonly JsonSerializerOptions s_debugJsonOptions = new() { WriteIndented = false }; // Compact JSON for logging
    ...
    private void EnsureDebugLogDirectoryExists()
    {
        ...
        {
            Console.Error.WriteLine($"[YTLiveChat DEBUG] Error ensuring debug log directory exists ('{_debugLogFilePath}'): {ex.Message}. Debug logging might fail.");
    ...
    private async Task StartAsync(string? handle, string? channelId, string? liveId, bool overwrite, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                OnErrorOccurred(new ErrorOccurredEventArgs(new ArgumentException("Failed to retrieve valid initial FetchOptions (LiveId is missing).")));
                OnChatStopped(new() { Reason = "Failed to get initial options" });
                return;
            }

            OnInitialPageLoaded(new() { LiveId = _fetchOptions.LiveId });
            Console.WriteLine($"[YTLiveChat] Initial page loaded for Live ID: {_fetchOptions.LiveId}. Starting polling...");
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    Console.Error.WriteLine("[YTLiveChat] Error: Fetch options or continuation token missing. Stopping poll.");
                ...
                if (!tick) break; // Timer stopped or task cancelled

                try
                {
                    // Use the instance field _fetchOptions directly
                    (LiveChatResponse? response, string? rawJson) = await httpClient.GetLiveChatAsync(_fetchOptions);
                    ...
                    if (_isDebugLoggingEnabled && response?.ContinuationContents?.LiveChatContinuation?.Actions != null)
                    {
                        ...
                        await LogRawJsonActionsAsync(response.ContinuationContents.LiveChatContinuation.Actions);
                    }
                    // --- End Log Raw JSON ---

                    if (response != null)
                    {
                        (List<Contracts.Models.ChatItem> items, string? continuation) = Parser.ParseLiveChatResponse(response);
                        ...
                        else
                        {
                            ...
                            Console.WriteLine("[YTLiveChat] No continuation token received in response. Stopping poll.");
                    ...
                    else
                    {
                        ...
                        OnErrorOccurred(new ErrorOccurredEventArgs(new Exception("Failed to fetch or deserialize live chat data.")));
                        Console.Error.WriteLine($"[YTLiveChat] Fetch/deserialize error occurred. See previous logs. Retrying after delay...");
                ...
                catch (Exception ex) // Catch specific exceptions? (e.g., HttpRequestException, JsonException)
                {
                    ...
                    if (ex is HttpRequestException httpEx && httpEx.StatusCode == System.Net.HttpStatusCode.Forbidden)
                    {
        ...
        {
            OnErrorOccurred(new ErrorOccurredEventArgs(ex));
            Console.Error.WriteLine($"[YTLiveChat] Critical error during initialization: {ex.Message}");
    ...
    {
        if (!_isDebugLoggingEnabled || actions == null) return;
        ...
        {
            Console.Error.WriteLine($"[YTLiveChat DEBUG] Error writing raw JSON actions to log file '{_debugLogFilePath}': {ex.Message}");
        }
        finally
        {
    ...
    protected virtual void OnInitialPageLoaded(InitialPageLoadedEventArgs e) => InitialPageLoaded?.Invoke(this, e);
    ...
    private async Task<FetchOptions?> GetOptionsAsync(string? handle = null, string? channelId = null, string? liveId = null, bool overwrite = false)
    {
        ...
        if (_fetchOptions == null || overwrite)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[YTLiveChat] Successfully parsed initial options. Live ID: {_fetchOptions?.LiveId}");
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"[YTLiveChat] Failed to get/parse initial options: {ex.Message}");
                _fetchOptions = null; // Ensure it's null on failure
                OnErrorOccurred(new ErrorOccurredEventArgs(new InvalidOperationException($"Failed to initialize from YouTube page: {ex.Message}", ex)));
            }
        }
        else
        {
